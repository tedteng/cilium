...
func getBlock(data []string, offset uint) (string, uint, error) {
	var block string
	var block_len uint64
	...
	for _, s := range data {
		if !have_length {
			...
			if index < 0 {
				...
				var err error
				block_len, err = strconv.ParseUint(block, 10, 64)
				if err != nil {
					return block, 0, err
				}
				if block_len == 0 {
					return block, 0, fmt.Errorf("Zero block length not allowed")
				}
				have_length = true
				missing = uint(block_len) + 1 // including the ':'
			}
		}
		if have_length {
